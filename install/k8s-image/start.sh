...
if [[ -z "${LAUNCH_AGENT_RUNNER_CONFIG_PATH}" ]]; then
  export LAUNCH_AGENT_RUNNER_CONFIG_PATH=/opt/circleci/launch-agent-config.yaml
fi

export LAUNCH_AGENT_RUNNER_DISABLE_AUTO_UPDATE=true

exec $prefix/circleci-launch-agent --config $LAUNCH_AGENT_RUNNER_CONFIG_PATH
